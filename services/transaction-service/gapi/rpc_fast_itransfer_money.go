...
import (
	...
	"github.com/tunvx/simplebank/transactions/gapi/val"
...
func (service *Service) FastInternalTransfer(ctx context.Context, req *pb.FastInternalTransferRequest) (*pb.FastInternalTransferResponse, error) {
	...
	senderAccount, err := service.getAndValidateAccount(ctx, req.SenderAccNumber, req.CurrencyType)
	if err != nil {
		return nil, err
	}
	...
	recipientAccount, err := service.getAndValidateAccount(ctx, req.RecipientAccNumber, req.CurrencyType)
	if err != nil {
		return nil, err
	}
	...
	result, err := service.store.CreateCompleteTx(ctx, arg)
	if err != nil {
