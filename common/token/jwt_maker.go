...
func (maker *JWTMaker) CreateToken(userID int64, sharID int32, role string, duration time.Duration) (string, *Payload, error) {
	payload, err := NewPayload(userID ,sharID, role, duration)
	if err != nil {
		return "", payload, err
	}
